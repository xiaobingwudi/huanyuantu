    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                # 尝试修复常见的JSON问题
                content = re.sub(r',\s*}', '}', content)  # 移除末尾多余的逗号
                content = re.sub(r',\s*]', ']', content)  # 移除数组末尾多余的逗号
                
                try:
                    return json.loads(content)
                except:
                    st.error("无法自动修复JSON，请从GitHub历史恢复数据")
                    # 返回空数据，但保留已有案例的备份
                    return {"cases": [], "error": "JSON corrupted"}
...
def load_html_annotations(file_content, filename):
    try:
        ...
        # 更精确地提取注释 - 只提取 <span id="note"> 中的内容
        ...
        if notes_match:
            ...
            # 清理HTML标签
            notes_text = re.sub(r'<[^>]+>', '', notes_text)  # 移除所有HTML标签
            notes_text = re.sub(r'&nbsp;', ' ', notes_text)   # 替换 &nbsp;
            notes_text = re.sub(r'&rsquo;', "'", notes_text)  # 替换 &rsquo;
            notes_text = re.sub(r'&[a-z]+;', ' ', notes_text)  # 移除其他HTML实体
            ...
            for line in lines:
                ...
                if match:
                    ...
                    # 清理注释文本
                    desc = clean_text_for_json(desc)
                    
                    if desc:  # 只保存非空注释
                        annotations[bar_num] = desc
        
        if not annotations:
            st.warning("未能解析到任何注释，请检查HTML格式")
            return {}, filename
...
def plot_kline(bars, annotations, first_bar_offset=0, title="K线图", show_edit_indicator=False):
    ...
    for i in range(len(bars)):
        ...
        y_pos_upper = row['high'] * 1.015 if is_green else row['low'] * 0.985
    ...
    for i in range(len(bars)):
        row = bars.iloc[i]
        is_green = row['close'] >= row['open']
        y_pos_lower = row['low'] * 0.985 if is_green else row['high'] * 1.015
        color = 'red' if is_green else 'black'
...
def save_case_to_database(case_id, title, date, start_time, end_time, df_pre, df_main, annotations):
    ...
    for original_num, desc in annotations.items():
        if 1 <= original_num <= main_count:
            # 清理注释文本，确保JSON兼容
            clean_desc = clean_text_for_json(desc)
            if clean_desc:  # 只保存非空注释
    ...
    # 清理标题
    clean_title = clean_text_for_json(title)
...
def create_kline_editor(df, prefix, max_bars=81):
    ...
    # 应用编辑
    if st.button(f"✅ 应用K线数据编辑", key=f"{prefix}_apply_edit", use_container_width=True):
        # 创建新的DataFrame
